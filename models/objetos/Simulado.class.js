...
class Simulado {
    ...
    static async listarPorUsuario(id) {
        const [rows] = await pool.query(
            "SELECT * FROM simulados WHERE feito_por = ? ORDER BY data DESC",
            [id]
        );
        return rows;
    }
    ...
    static async gerarSimulado(quantidade = 20, materia = null, tema = "") {
        // 1. Sortear IDs das questões
        const ids = await Questao.sortear(materia || "", tema || "", quantidade);

        if (!ids.length) return [];

        // 2. Buscar dados completos das questões sorteadas
        const questoes = [];
        for (const id of ids) {
    ...
    static async salvarResultado({ 
        feito_por, tempo, pontuacao, linguagens, exatas, ciencias_hum, ciencias_nat, quantidade_questoes 
    }) {
        const data = new Date();
    ...
    static async corrigir(respostasAluno) {
        ...
        respostasAluno.forEach(resp => {
            const escolhida = resp.escolhida ? String(resp.escolhida).trim().toUpperCase() : "";
            const correta = resp.correta ? String(resp.correta).trim().toUpperCase() : "";
    ...
    static async deletar(id) {
        await pool.query("DELETE FROM simulados WHERE id = ?", [id]);
        return true;
    }
}
